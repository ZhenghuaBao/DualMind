...
def is_english_article(url):
    ...
    if (
        ...
        or "intox" in url
        or "ces-photo" in url
        or "cette-photo" in url
        or "cette-image" in url
        or "ces-images" in url
    ):
        return False
    return True
...
def pesacheck_parser(url):
    ...
    if pub_date_tag:
        publication_date = "Publication Date:" + pub_date_tag.get(
            "content", "No publication date found"
        )
    else:
    ...
    try:
        text = (
            ...
            .split("This post is part of an ongoing series of PesaCheck")[0]
            .split("--")[1]
        )
    except:
        text = (
...
def factly_parser(url):
    ...
    text = (
        ...
        + publication_date
        + "\n"
        + "\n".join([p.get_text() for p in filtered_paragraphs]).split(
    ...
    image_urls = [
        i
        for i in image_urls
        if "logo" not in i.lower() and "thumbnail" not in i.lower()
    ][5:6]
    text += "\nImage URLs :\n" + "\n".join(image_urls)
    return text, image_urls


def collect_articles(urls, parser, scrape_images=True, image_urls=None, sleep=10):
    """
    ...
    for u in range(len(urls)):
        ...
        if is_new_article:
            ...
            scraped_image_urls = [
                img for img in scraped_image_urls if img not in img_urls_unique
            ]
...
def is_fc_organization(url):
    ...
    fc_domains = [
        ...
        "factuel.afp.com",
        "thequint.com",
        "logicalindian.com/fact-check/",
        "timesofindia.com/times-fact-check",
        "indiatoday.in/fact-check/",
        "smhoaxslayer.com",
        "facthunt.in",
        "aajtak.in/fact-check/",
        "bhaskar.com/no-fake-news",
        "theprint.in/hoaxposed/",
        "firstdraftnews.org",
    ]
    for d in fc_domains:
        if d in url:
...
def is_banned(url):
    ...
    banned = [
        ...
        "legalaiddc-prod.qed42.net",
        "windows8theme.net",
        "darkroom.baltimoresun.com",
        "dn.meethk.com",
        "hotcore.info",
        "pre-inscription.supmti.ac.ma",
        "hideaways.in",
        "www.alhurra.com/search?st=articleEx",
        "anonup.com",
        "hiliventures",
        "middleagerealestateagent",
        "nonchalantslip.fr",
        "corseprestige.com",
        ".aa.com.tr",
        "landing.rassan.ir",
...
def merge_data(evidence, evidence_metadata, dataset):
    ...
    merged_data = pd.merge(
        evidence_df,
        evidence_metadata_df.drop_duplicates(subset="raw url")[
            ["image path", "raw url"]
        ].rename(columns={"raw url": "url"}),
        on="url",
        how="inner",
    )
    merged_data = pd.merge(
        merged_data.rename(columns={"url": "evidence url"}),
        dataset_df[["org", "image path", "publication date"]].rename(
            columns={"publication date": "date_filter"}
        ),
        on="image path",
        how="inner",
    )
    ...
    fc_mask = merged_data.apply(
        lambda row: (
            False
            if row["org"] in row["evidence url"]
            or row["org"] in "".join(row["image url"])
            else True
        ),
        axis=1,
    )
    ...
    time_mask = merged_data.apply(
        lambda row: True if time_difference(row["date"], row["date_filter"]) else False,
        axis=1,
    )
...
def keep_longest_non_greyscale_area(image, color_threshold=30):
    ...
    def find_longest_sequence(arr, color_threshold):
        ...
        if len(start_indices) == 0 or (
            len(end_indices) > 0 and start_indices[0] > end_indices[0]
        ):
            start_indices = np.insert(start_indices, 0, 0)
        if len(end_indices) == 0 or (
            len(start_indices) > 0 and end_indices[-1] < start_indices[-1]
        ):
