...
def sort_with_clip_score(
    image_index, evidence_index_list, image_embeddings, clip_evidence_embeddings
):
    """
...
def sort_story_with_clip_score(
    evidence_index_list, clip_evidence_embeddings, clip_story_embeddings
):
    """
    ...
    for idx in evidence_index_list:
        sim = cosine_similarity(
            clip_evidence_embeddings[idx], clip_story_embeddings[idx]
        )
        similarities.append((idx, sim))
    # Sort by similarity in descending order
    sorted_indices = sorted(similarities, key=lambda x: x[1], reverse=True)
...
def sort_with_image_similarity(image_index, train_images_index_list, image_embeddings):
    """
...
def get_topk_evidence(
    image_path, evidence, image_embeddings, clip_evidence_embeddings, image_map, k=3
):
    """
    ...
    evidence_index = [
        evidence.index(ev) for ev in evidence if ev["image path"] == image_path
    ]

    if len(evidence_index) > k:
        image_index = int(image_map[image_path])
        sorted_evidence = sort_with_clip_score(
            image_index, evidence_index, image_embeddings, clip_evidence_embeddings
        )
...
def get_topk_keyword_evidence(
    ...
    #     print (ev['image path'])
    evidence = [
        ev for ev in evidence if ev["image path"] == image_path and ev["downloaded"]
    ]
    evidence_index = [
        evidence.index(ev)
        for ev in evidence
        if ev["image path"] == image_path and ev["downloaded"]
    ]
...
def get_topk_story(
    image_path, evidence, clip_evidence_embeddings, clip_story_embeddings, k=3
):
    """
    Given an image, get the topk keyword evidence using CLIP similarity.
    """
    ...
    if len(evidence_index) > k:
        sorted_evidence = sort_story_with_clip_score(
            evidence_index, clip_evidence_embeddings, clip_story_embeddings
        )
        # print (f'Sorted evidence: {sorted_evidence}')
        return sorted_evidence[:k]
    else:
...
def get_topk_demonstrations(
    image_path, question, train, image_embeddings, image_map, k=2
):
    """
    ...
    subset_train_index = [
        ...
        if train[t][question].lower() != "not enough information"
    ]
    ...
    for idx, i in zip(subset_train_index, [t["image path"] for t in subset_train]):
        try:
            train_image_idx.append(
                (idx, int(image_map[i]))
            )  # The candidate demonstrations index in the embedding matrix
        except:
            pass
    image_idx = int(
        image_map[image_path]
    )  # The index of the test image in the embedding matrix
    sorted_candidates = sort_with_image_similarity(
        image_idx, train_image_idx, image_embeddings
    )
...
def get_evidence_prompt(evidence):
    ...
    for ev in range(len(evidence)):
        ...
        if "image caption" and "image caption" in evidence[ev]:
            if (
                "Caption not found" not in evidence[ev]["image caption"]
                and "Image not found" not in evidence[ev]["image caption"]
            ):
...
def get_tokenized_evidence(evidence, tokenizer):
    ...
    for s in tqdm(range(len(evidence))):
        ...
        if (
            "Caption not found" not in evidence[s]["image caption"]
            and "Image not found" not in evidence[s]["image caption"]
        ):
...
def get_tokenized_keyword_evidence(evidence, tokenizer):
    ...
    for s in tqdm(range(len(evidence))):
        if evidence[s]["downloaded"]:
            ...
            cleaned_text = (
                tmp.replace("Caption not found", "").replace(";", " ").strip()
            )
            if cleaned_text:
                text += cleaned_text
            if text == "":
