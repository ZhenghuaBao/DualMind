...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Download articles and images based on provided URLs."
    )
    parser.add_argument(
        "--scrape_image",
        type=int,
    ...
    urls = (
        [t["URL"] for t in train] + [t["URL"] for t in val] + [t["URL"] for t in test]
    )
    image_urls = (
        [t["image URL"] for t in train]
        + [t[" image URL"] for t in val]
        + [t["image URL"] for t in test]
    )
    ...
    collect_articles(
        factly_urls, factly_parser, args.scrape_image, factlyu_image_urls, args.sleep
    )
    collect_articles(
        pesacheck_urls,
        pesacheck_parser,
        args.scrape_image,
        pesacheck_image_urls,
        args.sleep,
    )
    collect_articles(
        two11org_urls,
