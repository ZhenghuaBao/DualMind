...
if __name__=='__main__':
    ...
    parser.add_argument('--results_file', type=str, default='output/results_with_forgery-detection_motivation_gpt4o-mini.json',
                        help='Path to store the predicted answers.')
    parser.add_argument('--task', type=str, default='motivation',
    ...
    parser.add_argument('--forgery_detection_explanation', type=str, default='dataset/forgery_detection/FakeShield_output.jsonl',
                        help='Path to the forgery detection explanation file.')
    parser.add_argument('--forgery_detection_path', type=str, default='dataset/forgery_detection/FakeShield_output',
                        help='Path to the forgery detection output folder.')
    ...
    if args.modality in ['evidence','multimodal']:
        for i in range(len(image_paths)):
            evidence_idxs = get_topk_evidence(image_paths[i], evidence, image_embeddings, clip_evidence_embeddings, image_embeddings_map)
            evidence_idx.append(evidence_idxs)
            # print(len(evidence_idxs))
