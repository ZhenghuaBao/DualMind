...
def collect_data_wayback(
    ...
    url = (
        "http://web.archive.org/cdx/search/cdx?url=https://www."
        + website_url
        + "&collapse=digest&filter=!statuscode:404&showResumeKey=true&matchType=prefix&from=%s&to=%s&limit=%s&output=json"
    ...
    for _ in tqdm(range(its)):
        ...
        for i in range(1, len(parse_url) - 2):
            ...
            if len(orig_url.lower().split(website_url)) > 1:
                article_name = (
                    orig_url.lower().split(website_url)[1].split("/")[0].split("?")[0]
                )
            else:
        ...
        url = (
            ...
            + website_url
            + "&collapse=digest&showResumeKey=true&resumeKey="
            + resume_key
            + "&matchType=prefix&filter=!statuscode:404&from=%s&to=%s&limit=%s&output=json"
            % (start_date, end_date, chunk_size)
        )
        time.sleep(sleep)
    ...
    print(
        "Collected %s of the initial number of requested urls"
        % (round(len(url_list) / max_count, 2))
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Download articles and images from the Wayback machine."
    )
    parser.add_argument(
        "--url_domain",
        type=str,
    ...
    parser.add_argument(
        "--file_path",
        type=str,
        default="dataset/url/factly.txt",
        help="path to the file that stores the URLs",
    )
    parser.add_argument(
        "--parser",
        default=factly_parser,
    ...
    parser.add_argument(
        ...
        type=str,
        default="dataset/image_processing_instructions.txt",
    ...
    urls = [
        u
        for u in load_urls(args.file_path)
        if is_english_article(u) and re.search(pattern, u)
    ]
    ...
    if args.process_image:
        ...
        process_images_from_instructions(
            args.image_processing_script, "dataset/img/", "dataset/processed_img/"
        )
