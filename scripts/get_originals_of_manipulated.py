...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Heuristic to identify the unaltered, original image among the RIS results."
    )
    parser.add_argument(
        "--json_path",
        type=str,
        default="dataset/manipulation_detection_test.json",
    ...
    manipulation_detection_test_image_paths = [
        im["image path"]
        for im in load_json(args.json_path)
        if im["manipulation detection"] == "manipulated"
    ]
    if args.download_image:
        ...
        subset_evidence = [
            ev["image url"]
            for ev in evidence
            if ev["image path"] in manipulation_detection_test_image_paths
        ]
        ...
        image_to_download = [
            u.split(";")[0] for u in subset_evidence
        ]  # Take for each evidence the first version of the image
        for i in range(len(subset_evidence)):
            download_image(
                image_to_download[i],
                "dataset/manipulated_original_img/" + str(evidence_index[i]),
            )
    ...
    for img_path in manipulation_detection_test_image_paths:
        ...
        subset_index = [
            evidence.index(ev) for ev in evidence if ev["image path"] == img_path
        ]
        if len(subset) > 0:
            sorted_evidence_by_date_index = (
                pd.DataFrame(subset).sort_values(by="date").index.to_list()
            )
            if len(sorted_evidence_by_date_index) != 0:
                ...
                if str(idx) + ".png" in os.listdir("dataset/manipulated_original_img/"):
                    dict_original_image[img_path] = (
                        "dataset/manipulated_original_img/"
                        + str(subset_index[idx])
                        + ".png"
                    )
