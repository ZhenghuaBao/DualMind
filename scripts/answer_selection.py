...
if __name__ == "__main__":
    ...
    fallback_path = f"output/final_results_tineye/final_fallback_results_turbo_{task}.json"
    ...
    for idx, core_answer in enumerate(core_answers):
        ...
        invalid_answers = [ "not" "I can't", "I'm sorry", "I don't", "I'm unable to", "The image does not", "The photo does not", "The image provided does not", "Unknown", "Not specified"]
        if any(answer.lower() in core_answer["output"].strip().lower() for answer in invalid_answers):
            if not any(answer.lower() in fallback_answers[idx]["output"].strip().lower() for answer in invalid_answers):
    ...
    with open(f"output/final_results_tineye/final_complete_results_{task}.json", "w", encoding="utf-8") as file:
