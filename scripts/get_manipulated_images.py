...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Classify images as manipulated or not with a ViT model."
    )
    parser.add_argument(
        "--model_name",
        type=str,
    ...
    parser.add_argument(
        "--model_folder",
        type=str,
        default="baseline/vit-manipulation",
    ...
    parser.add_argument(
        "--learning_rate", type=int, default=2e-4, help="Learning rate for training"
    )
    ...
    train_target = [
        "manipulated" if im["type of image"] == "manipulated" else "non-manipulated"
        for im in train
    ]
    val_target = [
        "manipulated" if im["type of image"] == "manipulated" else "non-manipulated"
    ...
    train_dataset = Dataset.from_dict(
        {"image path": train_input, "labels": train_target}
    )
    ...
    test_predictions = [
        "non-manipulated" if p == 0 else "manipulated" for p in test_predictions
    ]
    results = [
        {
            "image path": test[im]["image path"],
            "manipulation detection": test_predictions[im],
        }
        for im in range(len(test))
    ]
