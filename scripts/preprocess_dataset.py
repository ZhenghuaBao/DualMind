...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Preprocess the raw data to create a train, val, and test sets."
    )
    parser.add_argument(
        "--json_file_path",
        type=str,
        default="dataset/gpt4_annotations/annotations.json",
    ...
    parser.add_argument(
        "--geonames_data",
        type=str,
        default="dataset/geonames_results.json",
    ...
    duplicates_mask = normalized_data["image path"].apply(
        lambda row: False if row in duplicates else True
    )
    normalized_data = normalized_data[duplicates_mask]
    # Remove images with no labeled pillars
    annotation_count = normalized_data[
        ["was the photo used before?", "source", "location", "date", "motivation"]
    ]
    for c in annotation_count.columns:
        annotation_count[c] = annotation_count[c].apply(
            lambda row: row if str(row).lower() != "not enough information" else None
        )
    ...
    normalized_data = normalized_data.fillna("not enough information").to_dict(
        orient="records"
    )
        ...
        for l in locations:
            if l not in all_locs:
                results = search_location(
                    l, args.geonames_username, args.sleep_geonames
                )
