...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--results_file",
        type=str,
        default="output/results_date.json",
    ...
    parser.add_argument(
        "--geonames_username",
        type=str,
        default=" ",  # Insert here your GeoName username. Necessary for location only
    ...
    for r in results:
        if args.task == "source":
            ...
            NER_ground_truth = [
                e
                for e in extract_named_entities(r["ground_truth"], nlp, "locations")
                if e in geonames_entries
            ]
            scores = evaluate(r["output"], r["ground_truth"], args.task)
